...
} function getData() { return JSON.parse(fs.readFileSync(DATA_FILE)); } function saveData(data) { fs.writeFileSync(DATA_FILE, JSON.stringify(data, null, 2)); } app.use(cors()); app.use(bodyParser.json()); app.use(express.static(path.join(__dirname, 'obfuscated'))); app.get('/api/data', (req, res) => { res.json(getData()); }); app.post('/api/admin/save', adminAuth, (req, res) => { const { executors, games } = req.body; const data = getData(); if (executors) data.executors = executors; if (games) data.games = games; saveData(data); io.emit('data_updated', data); res.json({ success: true }); }); io.on('connection', (socket) => { socket.emit('initial_data', getData()); }); app.get('/scripts/:scriptName', (req, res) => {
